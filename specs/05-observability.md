...
The same capture that powers `emissary:replay` (`llm_payloads`) doubles as a test-fixture source. `emissary:fixture:capture <turn_id>` freezes a captured turn into a JSON fixture that `AgentTestCase::replay()` re-runs deterministically as a Pest dataset — see Testing. Production incidents thus become permanent regression tests with no extra instrumentation.
