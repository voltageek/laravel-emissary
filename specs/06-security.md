...
### Data retention & PII
Conversation transcripts are sent to a third-party LLM and stored verbatim in `conversation_messages`. Configure a retention TTL and schedule `php artisan emissary:prune` to bound storage. The `content` column is not encrypted at rest by default — apps handling PII should encrypt it or avoid storing raw transcripts. *(See Data Models → Retention.)* Note the asymmetry: **channel credentials are secrets and ARE encrypted at rest** (see Channel Onboarding → Credential security), the opposite default to conversation content.
